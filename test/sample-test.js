...
describe("TestModifyVariable", function () {
  it("should change x to 1337", async function () {
    ...
    const contract = await ModifyVariable.deploy(10, "we");
  ...
  it("should change name to Nicolas", async function () {
    // this line creates an ethers ContractFactory abstraction: https://docs.ethers.org/v5/api/contract/contract-factory/
    const ModifyVariable = await ethers.getContractFactory("ModifyVariable");

    // we then use the ContractFactory object to deploy an instance of the contract
    const contract = await ModifyVariable.deploy(10, "helloword");

    // wait for contract to be deployed and validated!
    await contract.deployed();

    // modify name from constructor to Nicolas!
    await contract.changeUserName("Nicolas");
    // getter for state variable x
    const newName = await contract.username();
    assert.equal(newName, "Nicolas");
  });
});
